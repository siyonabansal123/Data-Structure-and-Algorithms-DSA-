...
def betterString(str1, str2):
    ...
    def countUniqueSubstrings(s):
        unique_substrings = []
        # Using nested loops to generate substrings
        for i in range(len(s)):
            for j in range(i + 1, len(s) + 1):
                print(unique_substrings)
                if s[i:j] not in unique_substrings:
                    unique_substrings.append(s[i:j])
        return len(unique_substrings)

    # Count unique substrings for both strings
    unique_substr1 = countUniqueSubstrings(str1)
    
    unique_substr2 = countUniqueSubstrings(str2)
    print(unique_substr1, unique_substr2)

    # Compare based on the number of unique substrings
    if (unique_substr1) >= (unique_substr2):
        return str1
    elif (unique_substr1) < (unique_substr2):
        return str2
    
    
# print(betterString("ljmolmti", "sqapzwbb"))

def perfectSum( arr, n, sums):
    # code here
    
    def subseq(arr, sums, ans=[], subarr=[], i=0):
        ...
        if i==len(arr) or sum(subarr)>sums:
            return ans
            
        subarr.append(arr[i])
        print(subarr)
        subseq(arr, sums, ans, subarr, i+1)
        subarr.pop()
        print(subarr)
        subseq(arr,  sums, ans, subarr, i+1)
    ...
    subseq(arr, sums, ans, subarr, i)
    
    return ans

print(perfectSum([5 ,2 ,3 ,10, 6, 8], 6, 10))
